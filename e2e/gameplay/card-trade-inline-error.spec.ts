...
test("game page shows an inline trade error and clears it on selection change", async ({
  page
}) => {
  ...
  await page.route("**/api/cards/trade", async (route) => {
    await route.fulfill({
      status: 400,
      json: { error: "Card set does not match a valid standard trade." }
    });
  });
  ...
  await expect(page.locator("#card-trade-error")).toContainText(

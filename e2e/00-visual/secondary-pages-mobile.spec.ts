...
test.describe("secondary pages mobile baselines", () => {
  ...
  test("authenticated profile mobile layout matches the baseline", async ({ page }) => {
    test.slow();
    await resetGame(page);
    await preferCommandTheme(page);
    await authenticateVisualUser(page, "visualmobile");
    ...
    await expect(page.getByTestId("player-profile-shell")).toHaveScreenshot(
      "profile-layout-mobile.png",
      {
        timeout: 15000,
        maxDiffPixels: 1200
      }

...
async function generate() {
  ...
  const rootPath = path.join(process.cwd(), 'env.js');
  await writeFile(rootPath, content, 'utf8');

  const distDir = path.join(process.cwd(), 'dist');
  if (fs.existsSync(distDir)) {
    try {
      await writeFile(path.join(distDir, 'env.js'), content, 'utf8');
    } catch (err) {
      console.error('Failed to write env.js to dist', err);
    }
  }
}

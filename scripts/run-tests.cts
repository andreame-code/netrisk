...
register("API ai join applica il rate limiting dopo troppi tentativi", async () => {
  await withServer(async (baseUrl) => {
    const owner = await createAuthenticatedSession(baseUrl, uniqueName("ai_limit_owner"));
    const created = await fetch(baseUrl + "/api/games", {
      method: "POST",
      headers: authHeaders(owner.sessionToken),
      body: JSON.stringify({
        name: "AI Throttle Match",
        totalPlayers: 4,
        players: Array.from({ length: 4 }, (_, i) => ({ type: i === 0 ? "human" : "human" }))
      })
    });
    assert.equal(created.status, 201);
    const createdPayload: any = await readJson(created);

    // Default max attempts is 5. We use rejoin for some to avoid lobby capacity limit of 4.
    for (let i = 0; i < 5; i++) {
      const res = await fetch(`${baseUrl}/api/ai/join`, {
        method: "POST",
        headers: authHeaders(owner.sessionToken),
        body: JSON.stringify({
          name: i < 3 ? `CPU ${i}` : `CPU 0`,
          gameId: createdPayload.game.id
        })
      });

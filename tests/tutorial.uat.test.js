describe("tutorial", () => {
  beforeEach(() => {
    ...
    document.body.innerHTML =
      '<div id="board"></div><div id="uiPanel"></div><div id="actionLog"></div>' +
  ...
  test("user can cancel and restart tutorial", () => {
    document.body.innerHTML +=
      '<button id="playTutorial"></button><button id="replayTutorial"></button>';
    const logSpy = jest.spyOn(console, "log").mockImplementation(() => {});
    jest.spyOn(window, "confirm").mockReturnValue(false);
    const startClick = jest.spyOn(
    ...
    tutorial.initTutorialButtons();
    expect(
      document.getElementById("playTutorial").classList.contains("hidden"),
    ).toBe(true);
  ...
  test("positions tutorial box within viewport", () => {
    ...
    board.dataset.rect = JSON.stringify({
      top: 560,
      left: 0,
      right: 100,
      bottom: 590,

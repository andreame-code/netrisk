describe('account page', () => {
  ...
  beforeEach(() => {
    ...
    jest.doMock('../src/init/supabase-client.js', () => ({ __esModule: true, default: supabaseMock }));
  ...
  test('loadUser fills DOM when user present', async () => {
    supabaseMock.auth.getUser.mockResolvedValue({
      data: { user: { id: '1', email: 'foo@example.com', user_metadata: { name: 'Foo' } } },
    });
  ...
  test('loadLobbies renders list', async () => {
    supabaseMock.auth.getUser.mockResolvedValue({ data: { user: { id: '1' } } });
    supabaseMock.from.mockImplementation(() => ({
      select: () => ({
        eq: () => ({ limit: () => Promise.resolve({ data: [{ code: 'AAA', map: 'M1' }] }) }),
        contains: () => ({ limit: () => Promise.resolve({ data: [{ code: 'BBB', map: 'M2' }] }) }),
      }),
    }));
    require('../src/account.js');
    await Promise.resolve();
  ...
  test('change password listener updates via API', async () => {
    ...
    await Promise.resolve();
    expect(supabaseMock.auth.updateUser).toHaveBeenCalledWith({ password: 'newpass' });
  ...
  test('update name listener updates DOM via API', async () => {
    ...
    await Promise.resolve();
    expect(supabaseMock.auth.updateUser).toHaveBeenCalledWith({ data: { name: 'New' } });

...
describe('ui integration', () => {
  ...
  test('world8 map renders eight starting territories', () => {
    destroyUI();
    const territoryDivs = world8.territories.map((t) => `<div id="${t.id}"></div>`).join('');
    document.body.innerHTML = `<div id="board"></div>${territoryDivs}`;
    const gameWorld = {
      players: world8.territories.map((_, i) => ({
        name: `P${i + 1}`,
        color: '#000',
      })),
      currentPlayer: 0,
      territories: world8.territories.map((t, i) => ({
        id: t.id,
        owner: i,
        armies: 1,
      })),
    ...
    const positions = Object.fromEntries(world8.territories.map((t) => [t.id, { x: t.x, y: t.y }]));

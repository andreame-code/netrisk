...
test.describe("lobby creation", () => {
        ...
        if (msg.type === "createLobby") {
          ws.send(
            JSON.stringify({
              type: "lobby",
              code: "1",
              host: msg.player?.name,
              maxPlayers: msg.maxPlayers,
              map: msg.map,
              players: [{ id: "p1", name: msg.player?.name }],
            }),
          );
        }
      });
  ...
  test("user can create a lobby", async ({ page }) => {
        ...
        body: `
          ...
          const supabase = {
            auth: {
              ...
              async getSession() {
                const raw = this.storage.getItem("session");
    ...
    await expect(page.locator('[data-testid="lobby-list"]')).toContainText('host: testuser');
  });
});

...
const mockSupabase = {
  auth: {
    getSession: jest.fn().mockResolvedValue({
      data: {
        session: {
          user: { id: 'p1', email: 'host@example.com', user_metadata: { username: 'host' } },
        },
      },
      error: null,
    }),
    getUser: jest
      .fn()
      .mockResolvedValue({ data: { user: { id: 'p1', user_metadata: { username: 'host' } } } }),
  },
...
describe('lobby flow', () => {
    ...
    class MockSocket {
      ...
      send(data) {
        ...
        if (msg.type === 'createLobby') {
          ...
          this.onmessage &&
            this.onmessage({
              data: JSON.stringify({ type: 'chat', id: msg.id, text: msg.text }),
            });
  ...
  test('creates lobby, exchanges chat, and shows error', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId('lobby-error').classList.contains('hidden'),
      ).toBe(false),
    );
  });
  ...
  test('reconnects using stored session', async () => {
    ...
    await waitFor(() =>
      expect(localStorage.getItem('lobbyCode')).toBe('ABCD'),
    );
    expect(
      screen.getByTestId('lobby-error').classList.contains('hidden'),
    ).toBe(true);

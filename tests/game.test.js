...
test('AI plays cards before reinforcing', () => {
  ...
  game.territoryById('t5').owner = 2;
  game.territoryById('t5').armies = 1;
  game.territoryById('t6').owner = 2;
  game.territoryById('t6').armies = 1;
  game.performAITurn();
...
test('players with no territories are skipped on turn rotation', () => {
  ...
  t3.owner = 1;
  t3.armies = 1; // defender's last territory
...
test('serialize and deserialize restores game state', () => {
  ...
  expect(restored.territoryById('t1').armies).toBe(game.territoryById('t1').armies);

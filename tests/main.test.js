...
jest.mock('../src/move-prompt.js', () => jest.fn(() => Promise.resolve(1)));
...
describe('main DOM interactions', () => {
  ...
  test('invalid player color does not crash updateUI', () => {
    ...
    const colorClasses = Array.from(t1.classList).filter((c) => c.startsWith('player-color-'));
  ...
  test('runAI processes AI turns', () => {
    ...
    const perform = jest.spyOn(main.game, 'performAITurn').mockImplementation(() => {

...
describe("ui utilities", () => {
  ...
  test("showVictoryModal displays winner info", () => {
    ...
    expect(document.getElementById("victoryTitle").textContent).toContain(
      "P1 has won",
    );
  });
  ...
  test("updateBonusInfo shows continent bonus", () => {
    updateBonusInfo();
    expect(document.getElementById("bonusInfo").textContent).toBe(
      "Bonus: c1 +2",
    );
  });
  ...
  test("go to move link highlights territories", () => {
    ...
    expect(document.getElementById("t1").classList.contains("highlight")).toBe(
      true,
    );
    jest.runAllTimers();
    expect(document.getElementById("t1").classList.contains("highlight")).toBe(
  ...
  test("updateInfoPanel displays AI profile", () => {
    ...
    updateInfoPanel();
    expect(document.getElementById("currentPlayer").textContent).toBe(
      "P1 (Easy/Aggressive)",
    );
  });
});

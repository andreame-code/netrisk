...
const { handleEventsRoute, handleStateRoute } = require("../../../backend/routes/game-read.cjs");
...
register(
  "handleStateRoute falls back to the session user when authorizeGameRead has no linked user",
  async () => {
    const calls: string[] = [];
    let sentPayload: Record<string, unknown> | null = null;
...
register(
  "handleEventsRoute keeps SSE reads non-mutating and streams the current snapshot",
  async () => {
    const calls: string[] = [];
    const writes: string[] = [];

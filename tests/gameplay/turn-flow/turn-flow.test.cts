...
function createAttackPhaseState(
  overrides: {
    playerArmies?: number;
    enemyArmies?: number;
...
function createFortifyPhaseState(
  overrides: {
    sourceArmies?: number;
    targetArmies?: number;
    fortifyUsed?: boolean;
...
register(
  "applyReinforcement transitions from reinforcement to attack when pool reaches zero",
  () => {
    const state = setupLiveGame();
    const currentPlayer = state.players[state.currentTurnIndex];
    const ownedTerritoryId = Object.keys(state.territories).find(
...
register(
  "endTurn blocca l'uscita dalla fase attacco finche il minimo modulare non e soddisfatto",
  () => {
...
register(
  "endTurn consente la fortifica se il minimo attacchi non e soddisfatto ma non esistono attacchi legali",
  () => {
    const state = createAttackPhaseState({
      playerArmies: 1,
      gameplayEffects: {
...
register(
  "endTurn blocca la chiusura del turno se il modulo richiede una fortifica e ne esiste una legale",
  () => {
...
register(
  "endTurn consente di chiudere il turno se la fortifica obbligatoria non e disponibile",
  () => {
    const state = createFortifyPhaseState({
      sourceArmies: 1,
      gameplayEffects: {
...
register("endTurn from fortify advances to the next active player reinforcement phase", () => {
  ...
  const ownedTerritoryId = Object.keys(state.territories).find(
    (territoryId: string) => state.territories[territoryId].ownerId === firstPlayer.id
  );
  if (!ownedTerritoryId) {
    throw new Error("Expected at least one owned territory for the current player.");
  }
...
register(
  "surrenderPlayer during the active turn hands off play to the next surviving player",
  () => {
    const state = setupLiveGame();
    state.players.push({ id: "p3", name: "Carol", color: "#333333", connected: true });
    const reassignedTerritoryId = Object.keys(state.territories).find(
      (territoryId: string) => state.territories[territoryId].ownerId === "p2"
    );
    ...
    const surrenderedTerritoryId = Object.keys(state.territories).find(
      (territoryId: string) => state.territories[territoryId].ownerId === currentPlayer.id
    );
    ...
    assert.equal(state.players[state.currentTurnIndex].id, "p2");
    assert.equal(
      state.players.find((player: PlayerRef) => player.id === currentPlayer.id).surrendered,
      true
    );

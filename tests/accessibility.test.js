...
describe('Accessibility features', () => {
  ...
  test('territories are accessible via keyboard with ARIA labels', async () => {
    ...
    pathA.dispatchEvent(new window.KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
  ...
  test('victory charts include ARIA labels', () => {
    ...
    expect(js).toMatch('canvas id="territoryChart" aria-label="Territories per turn" role="img"');
    expect(js).toMatch('canvas id="armiesChart" aria-label="Armies placed per turn" role="img"');
    expect(js).toMatch('canvas id="attackChart" aria-label="Attacks won and lost" role="img"');
  });
});

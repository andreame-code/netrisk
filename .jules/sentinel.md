...
## 2026-05-30 - Hardened API Request Parsing and Secure Error Propagation
**Vulnerability:** Mutation requests (POST/PUT/PATCH) did not enforce a specific Content-Type, potentially allowing CSRF bypasses or incompatible data parsing. Additionally, early-stage parsing errors (like invalid JSON or payload too large) were reported with inconsistent status codes and technical details.
**Learning:** In a custom HTTP server, request boundary conditions (Content-Type, size limits, JSON validity) must be explicitly enforced before hitting route logic to provide defense-in-depth. Masking internal server errors (500+) while allowing safe propagation of client errors (4xx) prevents information leakage while maintaining a good user experience.
**Prevention:** Enforce `application/json` for all mutation methods at the request entry point. Use a centralized error handler that propagates specific error attributes for 4xx errors but uses hardcoded fallbacks for 500+ errors.

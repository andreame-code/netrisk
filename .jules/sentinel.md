...
## 2026-05-27 - Hardened Security Headers and Robust Response Handling
**Vulnerability:** The application was missing standard rate-limiting headers (Retry-After), potential information disclosure (X-Powered-By), and unconditional HSTS (ignoring RFC 6797 recommendations for secure-only transport). Additionally, mock objects in certain regression tests caused TypeErrors when attempting to modify headers.
**Learning:** Security headers should be applied defensively at multiple levels (both global middleware and core response utilities). Furthermore, a custom server implementation must explicitly verify the existence of response methods (setHeader/removeHeader) before invocation to remain testable with mock objects while maintaining security.
**Prevention:** Always use defensive checks when interacting with the Node.js response object in core shared utilities. Ensure HSTS is conditionally applied to secure connections and include standard compliance headers like Retry-After in rate-limited paths.

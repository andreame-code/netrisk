  ...
  return {
    get(id) {
      if (!cache[id]) {
        ...
        cache[id] =
          document.querySelector(`.territory[data-id="${esc}"]`) || document.getElementById(id);
      }
      return cache[id];
    },
    clear() {
...
function updateInfoPanel() {
  ...
  if (cp) {
    ...
    if (colorClass) {
      const existing = Array.from(cp.classList).filter((c) => c.startsWith('player-color-'));
      existing.forEach((c) => cp.classList.remove(c));
      cp.classList.add(colorClass);
    }
  }
...
function addLogEntry(entry, meta = {}) {
  const logEntry = typeof entry === 'string' ? { message: entry, ...meta } : entry;
...
function updateBonusInfo() {
  ...
  const bonuses = game.continents
    .filter((c) => c.territories.every((id) => game.territoryById(id).owner === game.currentPlayer))
...
function getColorClass(color) {
  ...
  const exists = Array.from(colorStyleSheet.cssRules).some((r) => r.selectorText === selector);
  if (!exists) {
...
function updateTerritories(scale, playerColorClasses) {
  game.territories.forEach((t) => {
    ...
    const colorClasses = Array.from(el.classList).filter((c) => c.startsWith('player-color-'));

...
async function fetchLobbies() {
  if (!supabase) {
    ...
    showLobbyError(
      "Impossibile caricare la lista delle lobby. Riprova.",
      fetchLobbies,
    );
    return;
  }
...
export function initLobby() {
  ...
  if (!WS_URL && createBtn) {
    ...
    showLobbyError("Supabase non è configurato. Riprova.", () =>
      location.reload(),
    );
  }
  if (createBtn && dialog) {
  ...
  async function createGame(payload, dlg) {
    ...
    try {
      if (!url) {
        showLobbyError("Server multiplayer non disponibile. Riprova.", () =>
          location.reload(),
        );
        return;
      }
      if (!ws || ws.readyState !== WebSocket.OPEN) {
        ...
        ws.onclose = () =>
          showLobbyError(
            "Connessione al server multiplayer persa. Riprova.",
            () => createGame(payload, dlg),
          );
      } else {
        ws.send(
      ...
      showLobbyError("Impossibile creare la lobby. Riprova.", () =>
        createGame(payload, dlg),
      );
    }
  }
  ...
  if (storedCode && storedId) {
    ...
    if (url) {
      ...
      ws.onopen = () => {
        hideLobbyError();
        ws.send(
          JSON.stringify({ type: "reconnect", code: storedCode, id: storedId }),
        );
      };
  ...
  async function loadChatHistory() {
    ...
    try {
      ...
      (data || []).forEach((row) =>
        addChatMessage(row.id, row.text, new Date(row.created_at)),
      );
    } catch (err) {
  ...
  function handleMessage(e, dlg) {
    ...
    switch (msg.type) {
      ...
      case "error": {
        showLobbyError("Si è verificato un errore. Riprova.", () =>
          location.reload(),
        );
        break;
      }

...
function parseBody(req: Request): Promise<Record<string, any>> {
  ...
  if (requiresJson) {
    ...
    if (!contentType.includes("application/json")) {
      const error = createLocalizedError(
        "Content-Type non supportato (richiesto application/json).",
  ...
  return new Promise((resolve, reject) => {
    let raw = "";
    req.on("error", () => {
      const error = createLocalizedError("Errore nel caricamento payload", "server.bodyReadError");
    ...
    Promise.resolve()
      ...
      .catch((error: any) => {
        ...
        sendLocalizedError(
          ...
          error,
          "Errore interno.",
          error?.messageKey || "server.internalError"
        );
      });
  }

...
function scryptAsync(
  ...
  return new Promise((resolve, reject) => {
    crypto.scrypt(password, salt, keylen, (err: Error | null, derivedKey: Buffer) => {
      if (err) reject(err);
...
async function passwordRecord(
  ...
  const keylen = 64;
  const hash = (await scryptAsync(String(secret || ""), salt, keylen)).toString("hex");
  return {
    algorithm: "scrypt",
    salt,
    keylen,
...
async function verifyPassword(
  ...
  if (algorithm === "scrypt" || !hasValidRecord || !record?.digest) {
    ...
    candidate = (await scryptAsync(String(passwordToVerify || ""), salt, keylen)).toString("hex");
  } else {
    const iterations =
    ...
    candidate = (
      await pbkdf2Async(String(passwordToVerify || ""), salt, iterations, 32, digest)
    ).toString("hex");
  }

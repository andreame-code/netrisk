...
This matrix tracks the gradual migration to a 100% React frontend with safe dual-run support.
The canonical user-facing routes are now served by the React shell, while legacy pages remain available under `/legacy/*` as rollback-only surfaces until final cleanup.
...
| --- | --- | --- | --- | --- | --- | --- |
| `/` landing | marketing shell, top-nav session affordances, route handoff | session restore only | load, unauthenticated, authenticated, locale/theme application | `e2e/smoke/app-load.spec.ts`, layout visuals | canonical React landing is live; remaining work is documentation/polish rather than legacy dependency | Canonical landing stays React-served while `/legacy/*` remains rollback-only |
| `/register.html` | register form, login handoff, redirect to profile | `/api/auth/register`, `/api/auth/session` | load, validation error, success redirect, already-authenticated redirect | React register integration + smoke via shared auth flows | canonical React register is live; remaining legacy page is rollback-only | Canonical register stays React-served while `/legacy/*` remains rollback-only |

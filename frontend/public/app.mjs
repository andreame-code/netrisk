...
function fitMapBoardToViewport() {
    ...
    const stagePaddingX = Number.parseFloat(stageStyles.paddingLeft || "0") + Number.parseFloat(stageStyles.paddingRight || "0");
    const stagePaddingY = Number.parseFloat(stageStyles.paddingTop || "0") + Number.parseFloat(stageStyles.paddingBottom || "0");
...
function handleTerritoryClick(territoryId) {
    ...
    if (state.snapshot?.turnPhase === "fortify") {
        ...
        const canSelectFortifyTarget = state.fortifySelectionMode === "to" &&
            fortifySource &&
...
function render() {
    ...
    const attackTargets = (snapshot?.map || []).filter((territory) => source?.neighbors.includes(territory.id) &&
        territory.ownerId &&

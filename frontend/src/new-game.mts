...
function slotMarkup(index: number): string {
  if (index === 0) {
    return (
      '<div class="setup-slot is-fixed" data-slot-index="0">' +
      '<div class="setup-slot-head"><strong>' +
      t("newGame.slot.playerLabel", { number: index + 1 }) +
      '</strong><span class="badge accent">' +
      t("newGame.slot.creatorBadge") +
      "</span></div>" +
      '<div class="field-stack"><span>' +
      t("newGame.slot.typeLabel") +
      '</span><div class="setup-fixed-value">' +
      t("newGame.slot.humanOption") +
      "</div></div>" +
      '<p class="setup-slot-note" data-role="note">' +
      slotDescription("human", 0) +
      "</p>" +
      "</div>"
    );
  }
  ...
  return (
    '<div class="setup-slot" data-slot-index="' +
    index +
    '">' +
    '<div class="setup-slot-head"><strong>' +
    t("newGame.slot.playerLabel", { number: index + 1 }) +
    "</strong></div>" +
    '<label class="field-stack"><span>' +
    t("newGame.slot.typeLabel") +
    '</span><select data-role="type"><option value="human">' +
    t("newGame.slot.humanOption") +
    '</option><option value="ai">' +
    t("newGame.slot.aiOption") +
    "</option></select></label>" +
    '<p class="setup-slot-note" data-role="note">' +
    slotDescription("human", index) +
    "</p>" +
    "</div>"
  );
}
...
function renderSlots() {
  ...
  setMarkup(
    elements.playerSlots,
    Array.from({ length: total }, (_, index) => slotMarkup(index)).join("")
  );
  updateSlotNotes();
}
...
function setFeedback(message: string, type = ""): void {
  elements.feedback.className =
    "session-feedback" + (type === "error" ? " is-error" : "") + (message ? "" : " is-hidden");
...
function filterByAllowedIds<T extends { id: string }>(
  entries: T[],
...
function availableGamePresets(): NetRiskGamePreset[] {
  return state.gamePresets.filter(
    (preset) =>
...
function setSelectValueIfAvailable(
  select: HTMLSelectElement,
...
function selectedVictoryRuleSet(): VictoryRuleSet | null {
  return (
    state.victoryRuleSets.find((ruleSet) => ruleSet.id === elements.victoryRuleSet.value) || null
  );
}
...
function renderContentPackSummary() {
  ...
  const diceRuleSet =
    state.diceRuleSets.find((entry) => entry.id === contentPack.defaultDiceRuleSetId) || null;

  setMarkup(
    elements.contentPackSummary,
    '<div class="map-setup-card-head">' +
      "<strong>" +
      contentPack.name +
      "</strong>" +
      '<span class="badge">' +
      (map ? map.name : contentPack.defaultMapId) +
      "</span>" +
      "</div>" +
...
function renderRuleSetSummary() {
  ...
  setMarkup(
    elements.ruleSetSummary,
    '<div class="map-setup-card-head">' +
      "<strong>" +
      ruleSet.name +
      "</strong>" +
      '<span class="badge">' +
      diceRuleSetLabel(activeDiceRuleSet) +
      "</span>" +
      "</div>" +
      '<p class="map-setup-copy">' +
      t(
      ...
      '<span class="badge">' +
      namedOptionLabel(activeVictoryRuleSet) +
      "</span>" +
      '<span class="badge">' +
      namedOptionLabel(activeTheme) +
      "</span>" +
      '<span class="badge">' +
      namedOptionLabel(activePieceSkin) +
      "</span>" +
      '<span class="badge">' +
      (selectedMapSummary()?.name || t("common.notAvailable")) +
      "</span>" +
...
function gamePresetSelectMarkup(presets: NetRiskGamePreset[]): string {
  return (
    '<label class="field-stack"><span>Preset modulo</span><select id="setup-game-preset">' +
    '<option value="">' +
...
function applySelectedGamePreset(presetId: string): void {
  ...
  Array.from(document.querySelectorAll<HTMLInputElement>("[data-module-checkbox]")).forEach(
    (checkbox) => {
      checkbox.checked = Boolean(
        preset &&
        Array.isArray(preset.activeModuleIds) &&
        preset.activeModuleIds.includes(checkbox.value)
      );
    }
  );
  ...
  const contentProfileSelect = document.querySelector(
    "#setup-content-profile"
  ) as HTMLSelectElement | null;
  const gameplayProfileSelect = document.querySelector(
    "#setup-gameplay-profile"
  ) as HTMLSelectElement | null;
...
function refreshSelectableCatalogs(): void {
  ...
  const availableContentPackOptions = filterByAllowedIds(
    state.contentPacks,
    selectedContent.contentPackIds
  ).map((pack) => ({ value: pack.id, label: pack.name }));
  const availableMapOptions = filterByAllowedIds(state.maps, selectedContent.mapIds).map((map) => ({
    value: map.id,
    label: map.name
  }));
  const availableDiceOptions = filterByAllowedIds(
    state.diceRuleSets,
    selectedContent.diceRuleSetIds
  ).map((ruleSet) => ({ value: ruleSet.id, label: diceRuleSetLabel(ruleSet) }));
  const availableVictoryOptions = filterByAllowedIds(
    state.victoryRuleSets,
    selectedContent.victoryRuleSetIds
  ).map((ruleSet) => ({ value: ruleSet.id, label: ruleSet.name }));
  const availableThemeOptions = filterByAllowedIds(state.themes, selectedContent.siteThemeIds).map(
    (theme) => ({ value: theme.id, label: theme.name })
  );
  const availablePieceSkinOptions = filterByAllowedIds(
    state.pieceSkins,
    selectedContent.pieceSkinIds
...
function renderModuleOptions() {
  ...
  if (
    ...
    !gamePresets.length &&
    !state.contentProfiles.length &&
    !state.gameplayProfiles.length &&
    !state.uiProfiles.length
  ) {
    setMarkup(container, "");
        ...
        ? visibleModules
            .map(
              (moduleEntry) =>
                ...
                '<span class="setup-slot-head"><strong>' +
                moduleEntry.displayName +
                '</strong><span class="badge">' +
                (moduleEntry.kind || "module") +
                "</span></span>" +
                '<span class="map-setup-copy">' +
                (moduleEntry.description || "") +
                "</span>" +
                '<input type="checkbox" data-module-checkbox value="' +
                moduleEntry.id +
                '"' +
                (selectedIds.has(moduleEntry.id) ? " checked" : "") +
                " />" +
                "</label>"
            )
            .join("")
  ...
  if (
    contentProfileSelect &&
    contentProfiles.some((profile) => profile.id === selectedContentProfileId)
  ) {
    contentProfileSelect.value = selectedContentProfileId;
  }
  if (
    gameplayProfileSelect &&
    gameplayProfiles.some((profile) => profile.id === selectedGameplayProfileId)
  ) {
  ...
  if (
    gamePresetSelect &&
    state.selectedGamePresetId &&
    gamePresets.some((preset) => preset.id === state.selectedGamePresetId)
  ) {
...
function readConfig() {
  ...
  const players = Array.from(elements.playerSlots.querySelectorAll("[data-slot-index]")).map(
    (slot, index) => ({
      type:
        index === 0
          ? "human"
          : (slot.querySelector('[data-role="type"]') as HTMLSelectElement | null)?.value ||
            "human",
      slot: index + 1
    })
  );
  ...
  return {
    ...
    ...(selectedProfileValue("content")
      ? { contentProfileId: selectedProfileValue("content") }
      : {}),
    ...(selectedProfileValue("gameplay")
      ? { gameplayProfileId: selectedProfileValue("gameplay") }
      : {}),
    ...(selectedProfileValue("ui") ? { uiProfileId: selectedProfileValue("ui") } : {}),
    ...(elements.turnTimeoutHours.value
      ? { turnTimeoutHours: Number(elements.turnTimeoutHours.value) }
      : {}),
    totalPlayers,
    players
  };
...
async function loadOptions() {
  ...
  setSelectOptions(
    elements.ruleSet,
    state.ruleSets.map((ruleSet) => ({ value: ruleSet.id, label: ruleSet.name }))
  );
  refreshSelectableCatalogs();
  setSelectOptions(
    elements.turnTimeoutHours,
    state.turnTimeoutHoursOptions.map((hours) => ({
      value: String(hours),
